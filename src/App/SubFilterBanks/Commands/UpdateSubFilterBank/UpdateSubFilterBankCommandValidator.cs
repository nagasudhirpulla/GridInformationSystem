...
public class UpdateSubFilterBankCommandValidator : AbstractValidator<UpdateSubFilterBankCommand>
{
    private readonly IApplicationDbContext _context;

    public UpdateSubFilterBankCommandValidator(IApplicationDbContext context)
    {
        _context = context;
    ...
    public async Task<bool> BeUniqueSubFilterBankInSubstation(UpdateSubFilterBankCommand cmd, CancellationToken cancellationToken)
    {
        bool sameSubFilterBankExists = await _context.SubFilterBanks
            .AnyAsync(l => (l.Id != cmd.Id) && (l.FilterBankId == cmd.FilterBankId) && (l.ElementNumber == cmd.SubFilterTag), cancellationToken);
